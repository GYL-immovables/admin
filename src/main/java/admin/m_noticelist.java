...
public class m_noticelist {
	...
	public ArrayList<ArrayList<String>> db_data(){
		try {
			this.con = m_dbinfo.dbinfo();
			//필요한 컬럼만 지정하여 select 문법으로 데이터를 가져오는 코드 
			this.sql = "select nidx,nsubject,writer,ndate,nview,(select count(*) from notice) as total from notice order by nidx desc limit ?,?";
